...
Bar_Graph <- Master_Bar_Summary %>% 
  ...
  labs(x = "Structure",
       ...
       fill = "Group",
       title = "Average (SD) of Target-Like Responses") +
...
Boxplot <- Master_Box %>% 
  ...
  labs(x = "Structure",
       fill = "Group",
       title = "Distribution of Target-Like Responses") +
  theme(axis.title = element_text(face = "bold"),
        plot.title = element_text(hjust = 0.5, face = "bold"),
...
# Merge graphs
Final_Graph <- (Bar_Graph + Boxplot)

Final_Graph

ggsave(filename = here("Conferences", "HLRI", "HLRI Graph.pdf"),
       plot = Final_Graph,
       device = "pdf",
       width = 16,
       height = 4,
