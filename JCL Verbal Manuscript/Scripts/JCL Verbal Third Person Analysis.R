...
# Joint table
Agreement_Results_Table <- rbind(Third_Status_Table, Third_Bilingualism_Table) %>% 
  rename(Effect = term,
         Estimate = estimate,
         SE = std.error,
         p = p.value) %>% 
  ...
  mutate(Estimate = round(Estimate, 2),
         SE = round(SE, 2),
         p = round(p, 5)) %>% 
  write_csv(here("JCL Verbal Manuscript", "Results", "JCL Verbal Third Person GLMM Results.csv"))
